      ...
      return (
        <Link href={`/coins/${currency.uuid}`} key={index}>
          <tr className="border-y-2 border-indigo-50 cursor-pointer">
            <td className="py-2 pl-24 font-medium w-21" style={{ width: '12%' }}>{(index + 1)}</td>
            <td className="py-2 w-12" style={{ width: '5%' }}><img src={currency.iconUrl} style={{ maxHeight: '30px', maxWidth: '30px' }} alt="cryptoIcon" /></td>
            ...
            <td className="py-2 font-medium" style={{ width: '20%' }}>$ {millify(currency.marketCap, {
              precision: 2,
              units: ['', 'K', 'Million', 'Billion', 'Tillion', 'P', 'E'],
              space: true,
            })}</td>
            <td className="py-2 pr-14" style={{ width: '18%' }}><PercentageBetterment percentage={currency.change}/></td>
          </tr>
        </Link>
      );
    })
  ...
  return (
    <div className="flex h-full flex-col tracking-wide text-indigo-900">
      <div>
        ...
        <div className='flex-auto max-w-screen-xl bg-white border-b-2 border-indigo-50 px-8 py-4 mx-auto sticky top-0 z-20'>
        ...
        <div className='flex-auto flex-col max-w-screen-xl flex justify-center items-center mx-auto mt-10  '>
          <Image
            ...
            width={230}
            height={230} />
          <h1 className='flex text-4xl font-bold  text-indigo-600 mt-2'>Cryptocurrency Research Lab</h1>
          <div className="pt-5 pb-8 text-center">Everyday new project launches, hyping up the market with perfect numbers, <br />futuristic arts and visionary roadmaps. But investment is not just about word of mouth. <br />Do You Own Research and made your own investment decision.</div>
        ...
        <div className="flex-auto mt-6 h-fit max-w-screen-xl justify-center mx-auto">
          <table class="table-auto" style={{ width: "100%" }} >
            <thead>
              <tr className='text-left tracking-widest text-indigo-600'>
                <th key="cryptocurrency" className=" pl-24" colspan="3">CRYPTOCURRENCY</th>
                <th key="price" className="">PRICE</th>
                <th key="marketcap" className="">MARKET CAP</th>
                <th key="24h" className="pr-14">24H</th>
              </tr>
            </thead>
            <tbody>
              {renderList()}
